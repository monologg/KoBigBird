...
def get_linear_warmup_linear_decay_lr(init_lr, num_train_steps, num_warmup_steps):
...
def get_optimizer(params, learning_rate):
    ...
    if params["optimizer"] == "Adafactor":
        try:
            ...
            logging.error("tensor2tensor not installed. Cannot use Adafactor." "Defaulting to Adam.")
...
class AdamWeightDecayOptimizer(tf.compat.v1.train.Optimizer):
    ...
    def _apply_dense(self, grad, var):
        ...
        return tf.group([var.assign(next_param), m.assign(next_m), v.assign(next_v)])
    ...
    def _apply_sparse(self, grad, var):
        ...
        return self._apply_sparse_shared(grad.values, grad.indices, var, scatter_update_fn)

    def _resource_apply_sparse(self, grad, var, indices):
        """See `tf.train.Optimizer._resource_apply_spase()`."""

        def scatter_update_fn(x, i, v):
            with tf.control_dependencies([resource_variable_ops.resource_scatter_update(x.handle, i, v)]):
                return x.value()
