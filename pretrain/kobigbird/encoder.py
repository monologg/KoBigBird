...
from kobigbird import attention, recompute_grad, utils
    ...
    def __init__(
        ...
        with tf.compat.v1.variable_scope(name):
            ...
            with tf.compat.v1.variable_scope("attention"):
                ...
                # Dropout
                self.attention_dropout = recompute_grad.RecomputingDropout(hidden_dropout_prob)
...
class EncoderStack(tf.keras.layers.Layer):
    ...
    def call(self, encoder_inputs, encoder_inputs_mask, training=None):
        ...
        if self.params["attention_type"] == "block_sparse":
            ...
            encoder_from_mask = tf.reshape(encoder_inputs_mask, (-1, 1, encoder_length, 1))
            encoder_to_mask = tf.reshape(encoder_inputs_mask, (-1, 1, 1, encoder_length))

            # create band padding
            band_mask = attention.create_band_mask_from_inputs(blocked_encoder_mask, blocked_encoder_mask)
            ...
            encoder_inputs_mask = tf.cast(encoder_inputs_mask, tf.float32)
            attention_mask = attention.create_attention_mask_from_input_mask(encoder_inputs_mask, encoder_inputs_mask)
