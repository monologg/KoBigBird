...
def beam_search(
    ...
    def _loop_body(
        ...
        log_probs_BxMxV = logits_BxMxV - tf.reduce_logsumexp(logits_BxMxV, axis=2, keepdims=True)
        ...
        new_finished_flags_Bx2M = tf.cast(tf.reduce_any(tf.equal(new_seq_Bx2MxT, eos_id), axis=-1), dtype)
    ...
    # process finished.
    final_finished_flag_BxMx1 = tf.reduce_any(tf.equal(final_finished_seq_BxMxT, eos_id), axis=-1, keepdims=True)
