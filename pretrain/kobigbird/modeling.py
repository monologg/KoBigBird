...
from kobigbird import decoder, encoder, utils
...
class BertModel(tf.keras.layers.Layer):
    ...
    def __init__(self, params):
        ...
        if self.params["attention_type"] == "simulated_sparse" or self.params["attention_type"] == "block_sparse":
            if params["max_encoder_length"] % params["block_size"]:
                logging.info("Expand max_encoder_length to next multiple of block_size")
                self.params["max_encoder_length"] = (params["max_encoder_length"] // params["block_size"] + 1) * params[
                    "block_size"
                ]
                pad_size = self.params["max_encoder_length"] - params["max_encoder_length"]
        ...
        with tf.compat.v1.variable_scope(self.scope, reuse=tf.compat.v1.AUTO_REUSE):
            self.embeder = utils.EmbeddingLayer(
                ...
                use_token_type=self.params["use_token_type"],
    ...
    def call(self, input_ids, token_type_ids=None, training=None):
        ...
        # Generate mask.
        input_mask = tf.where(
            input_ids != self.params["pad_token_id"], tf.ones_like(input_ids), tf.zeros_like(input_ids)
        )
...
class TransformerModel(tf.keras.layers.Layer):
    ...
    def _encode(self, input_ids, training=None):
        ...
        # Perform embedding lookup on the word ids.
        input_embs = self.embeder(input_ids, self.params["max_encoder_length"], training=training)
    ...
    def _get_start_token_ids(self, tensor_for_shape):
        ...
        return tf.ones([batch_size], dtype=tf.int32) * self.params["bos_token_id"]
    ...
    def get_inputs_from_targets(self, targets, start_token_ids):
        ...
        mask = tf.sequence_mask(length, self.params["max_decoder_length"] + 1, dtype=tf.int32)
    ...
    def _decode(
        ...
        input_embs = self.embeder(input_ids, self.params["max_decoder_length"], training=training)

        outputs = self.decoder(input_embs, target_mask, encoder_output, encoder_mask, training=training)

        logits = self.embeder.linear(outputs)
        output_ids = tf.cast(tf.argmax(logits, axis=-1), tf.int32)

        log_probs = -tf.nn.sparse_softmax_cross_entropy_with_logits(labels=target_ids, logits=logits)
        log_probs = tf.where(target_ids > self.params["pad_token_id"], log_probs, tf.zeros_like(log_probs, tf.float32))
    ...
    def _get_symbols_to_logits_fn(self, decoder_self_attention_mask):
        ...
        def _symbols_to_logits_fn(target_ids, cache, i):
            ...
            self_attention_mask = tf.slice(decoder_self_attention_mask, [0, 0, i, 0], [1, 1, 1, max_decode_len])

            # Preprocess decoder input by getting embeddings and adding timing signal.
            decoder_input = self.embeder(decoder_input, 1, start_pos=i, training=False)
    ...
    def _predict(self, target_ids, target_mask, start_token_ids, encoder_output, encoder_mask):
        ...
        end_token_id = self.params["eos_token_id"]
    ...
    def _decode_and_predict(self, target_ids, encoder_output, encoder_mask, training=None):
        ...
        target_mask = decoder.create_self_attention_mask(self.params["max_decoder_length"])
        ...
        if training:
            ...
            predictions = self._predict(target_ids, target_mask, start_token_ids, encoder_output, encoder_mask)
    ...
    def call(self, input_ids, target_ids=None, training=None):
        ...
        # Decode.
        predictions = self._decode_and_predict(target_ids, encoder_output, encoder_mask, training=training)
