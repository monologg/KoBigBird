...
class Dense3dLayer(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        with tf.compat.v1.variable_scope(name):
            ...
            if self.use_bias:
                self.b = tf.compat.v1.get_variable(name="bias", shape=[hidden_size], initializer=tf.zeros_initializer())
            else:
                self.b = None
    ...
    def call(self, input_tensor):
        ...
        """
        hidden_size = self.num_attention_heads * self.size_per_head
        reshape_w = tf.reshape(self.w, [hidden_size, self.num_attention_heads, self.size_per_head])
        if self.head_first:
        ...
        if self.use_bias:
            if self.head_first:
                reshape_b = tf.reshape(self.b, [1, self.num_attention_heads, 1, self.size_per_head])
            else:
                reshape_b = tf.reshape(self.b, [self.num_attention_heads, self.size_per_head])
            ret += reshape_b
...
class Dense3dProjLayer(tf.keras.layers.Layer):
    ...
    def call(self, input_tensor):
        ...
        """
        hidden_size = self.num_attention_heads * self.size_per_head
        reshape_w = tf.reshape(self.w, [self.num_attention_heads, self.size_per_head, hidden_size])
...
class Dense2dLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, input_size, output_size, initializer, activation, name=None, use_bias=True):
...
class NormLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, hdim, dtype=tf.float32, name="LayerNorm"):
        ...
        with tf.compat.v1.variable_scope(name):
            self.beta = tf.compat.v1.get_variable("beta", [hdim], dtype=dtype, initializer=tf.zeros_initializer())
            self.gamma = tf.compat.v1.get_variable("gamma", [hdim], dtype=dtype, initializer=tf.ones_initializer())
...
class EmbeddingLayer(tf.keras.layers.Layer):
    ...
    def call(self, input_ids, seq_length, start_pos=0, token_type_ids=None, training=None):
        ...
        if self.token_type_table is not None:
            ...
            token_type_embeddings = tf.tensordot(one_hot_ids, self.token_type_table, 1)
...
def get_estimator(config, model_fn):
    ...
    if config["use_tpu"]:
        ...
        session_config = tf.compat.v1.ConfigProto(
            ...
            gpu_options=tf.compat.v1.GPUOptions(per_process_gpu_memory_fraction=1.2),
        )
        cluster_resolver = tf.distribute.cluster_resolver.TFConfigClusterResolver()
        with tf.compat.v1.Session(cluster_resolver.master(), config=session_config) as sess:
    ...
    if config["init_checkpoint"]:
        ...
        ckpt_var_list = {
            ...
            if not re.findall(
                "(Adam|Adafactor|global_step|adam_m|adam_v|AdamWeightDecayOptimizer|AdamWeightDecayOptimizer_1)", name
            )
        }
        ckpt_var_list["bert/encoder/LayerNorm/beta"] = ckpt_var_list["bert/embeddings/LayerNorm/beta"]
        ckpt_var_list["bert/encoder/LayerNorm/gamma"] = ckpt_var_list["bert/embeddings/LayerNorm/gamma"]

        # Because orignal bert's pos emb is 512, we random init pos emb which is bigger than 512 (e.g. 4096)
        if config["random_pos_emb"]:
            logging.info("*" * 40)
            logging.info("Random initialize the positional embedding!!")
            logging.info("*" * 40)
            filtered_ckpt_var_list = []
            for ckpt_var in ckpt_var_list.keys():
                if "embeddings/position_embeddings" not in ckpt_var:
                    filtered_ckpt_var_list.append(ckpt_var)
            vars_to_warm_start = "({})".format("|".join(filtered_ckpt_var_list))
        else:
        ...
        warm_start_settings = tf.estimator.WarmStartSettings(
            ...
            var_name_to_prev_var_name={
                "bert/encoder/LayerNorm/beta": "bert/embeddings/LayerNorm/beta",
                "bert/encoder/LayerNorm/gamma": "bert/embeddings/LayerNorm/gamma",
            },
        )
    else:
        ckpt_var_list = {}
...
def log_variables(variables, ckpt_var_list):
    ...
    logging.info("The model has {} trainable variables " "({:,} parameters):\n".format(len(model_var_list), num_params))
    ...
    if ckpt_var_list:
        logging.warning(
            "The warm start checkpoint contained %d variables that were not used " "for the model:\n",
            len(ckpt_var_list),
        )
